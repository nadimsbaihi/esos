...
exit:
    ...
    if (respond_status != FWK_SUCCESS) {
        rt_kprintf("[SCMI-PERF] %s @%d", __func__, __LINE__);
    }
    ...
    const uint32_t *payload, size_t payload_size, unsigned int message_id)
{
    ...
    if (FWK_ARRAY_SIZE(handler_table) !=
        FWK_ARRAY_SIZE(payload_size_table)) {
        rt_kprintf("[SCMI] Performance management protocol table sizes not consistent");
	while (1);
    }
...
static int scmi_init_notifications(int domains)
{
    ...
    if (scmi_perf_ctx.agent_count == 0u) {
    	rt_kprintf("%s:%d, hang\n", __func__, __LINE__);
	while (1);
    }
