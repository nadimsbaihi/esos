    ...
    const uint32_t *payload)
{
    ...
    for (index = 0,
	    ...
	    if (shared_scmi_ctx->config->dis_protocol_list_psci == NULL &&
			    shared_scmi_ctx->config->dis_protocol_count_psci != 0) {
	    	rt_kprintf("%s:%d, hang\n", __func__, __LINE__);
		while (1);
	    }
...
error:
    ...
    if (respond_status != FWK_SUCCESS) {
        rt_kprintf("[SCMI_BASE] %s @%d", __func__, __LINE__);
    }
    ...
    unsigned int message_id)
{
    ...
    if (FWK_ARRAY_SIZE(base_handler_table) !=
            FWK_ARRAY_SIZE(base_payload_size_table)) {
	    
        rt_kprintf("[SCMI] Base protocol table sizes not consistent");
	while (1);
