    ...
    const void *payload, size_t size)
{
    ...
    if (status != FWK_SUCCESS) {
        rt_kprintf("[SCMI] %s @%d", __func__, __LINE__);
    }
}
    ...
    fwk_id_t service_id)
{
    ...
    if (operation_id >= MOD_SCMI_PROTOCOL_MAX_OPERATION_ID) {
    	rt_kprintf("%s: %d, hang\n", __func__, __LINE__);
	while (1);
    }
    /*
    ...
    if (subscribers->operation_id_to_idx[operation_id] ==
        MOD_SCMI_PROTOCOL_OPERATION_IDX_INVALID) {
        if (subscribers->operation_idx >= subscribers->operation_count) {
		rt_kprintf("%s: %d, hang\n", __func__, __LINE__);
		while (1);
	}
                              ...
                              struct fwk_event *resp)
{
    ...
    if (status != FWK_SUCCESS) {
#if FWK_LOG_LEVEL <= FWK_LOG_LEVEL_ERROR
        rt_kprintf("[SCMI] %s: Unable to read message header", service_name);
#else
        (void)service_name;
#endif
        return status;
    ...
    if (status != FWK_SUCCESS) {
#if FWK_LOG_LEVEL <= FWK_LOG_LEVEL_ERROR
        rt_kprintf("[SCMI] %s: Unable to read message payload", service_name);
#else
        (void)service_name;
#endif
        return status;
    ...
    if (status != FWK_SUCCESS) {
#    if FWK_LOG_LEVEL <= FWK_LOG_LEVEL_ERROR
        rt_kprintf("[SCMI] %s: Unable to get agent id", service_name);
#    endif
        return status;
    }
    ...
    if (status != FWK_SUCCESS) {
#    if FWK_LOG_LEVEL <= FWK_LOG_LEVEL_ERROR
        rt_kprintf("[SCMI] %s: Unable to get agent type", service_name);
#    endif
        return status;
    }
