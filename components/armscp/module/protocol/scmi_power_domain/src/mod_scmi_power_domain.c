                                            ...
                                            const uint32_t *payload)
{
    ...
    if (respond_status != FWK_SUCCESS) {
        rt_kprintf("[SCMI-power] %s @%d", __func__, __LINE__);
    }
    ...
    const uint32_t *payload, size_t payload_size, unsigned int message_id)
{
    ...
    if (FWK_ARRAY_SIZE(handler_table) != FWK_ARRAY_SIZE(payload_size_table)) {
    	rt_kprintf("[SCMI] Power domain management protocol table sizes not consistent\n");
	while (1);
    }

    if (payload == NULL) {
    	rt_kprintf("non-avaible payload ptr\n");
	while (1);
    }
