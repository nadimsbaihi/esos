    ...
    const uint32_t *payload)
{
    ...
    if (agent->domain_count >= SCMI_VOLTD_PROTOCOL_ATTRIBUTES_VOLTD_COUNT_MAX) {
...
exit:
    ...
    if (respond_status != FWK_SUCCESS) {
	    rt_kprintf("[SCMI-VOLT] %s @%d", __func__, __LINE__);
    }
    ...
    const uint32_t *payload, size_t payload_size, unsigned int message_id)
{
    ...
    if (FWK_ARRAY_SIZE(handler_table) != FWK_ARRAY_SIZE(payload_size_table)) {
	    rt_kprintf("[SCMI] Inconsistent voltage domain management protocol table sizes");
	    while (1);
    }
