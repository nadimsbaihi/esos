...
static void graceful_timer_callback(uintptr_t mod_scmi_system_state)
{
    ...
    rt_kprintf("SCMI_SYS_POWER: Graceful request timeout...");
    rt_kprintf(
    ...
    if (mod_scmi_system_state == SCMI_SYSTEM_STATE_SHUTDOWN) {
        ...
        if (status != FWK_SUCCESS) {
            rt_kprintf("SCMI_SYS_POWER: %s @%d", __func__, __LINE__);
        }
    }
}
#endif
                                  ...
                                  unsigned int message_id)
{
    ...
    if (FWK_ARRAY_SIZE(handler_table) != FWK_ARRAY_SIZE(payload_size_table)) {
         rt_kprintf("[SCMI] System power protocol table sizes not consistent");
    }
