    ...
    struct mod_mock_voltage_domain_element_ctx **ctx)
{
    ...
    if (module_idx == FWK_MODULE_IDX_PMIC_VOLTAGE_DOMAIN) {
        if (ctx != NULL) {
...
static void mod_mock_voltage_domain_alarm_callback(uintptr_t element_idx)
{
    ...
    fwk_id_t element_id =
        fwk_id_build_element_id(fwk_module_id_pmic_voltage_domain, element_idx);
    ...
    if (status == FWK_SUCCESS) {
        ...
        case MOD_MOCK_VOLTAGE_DOMAIN_REQUEST_SET_VOLTAGE:
            ...
            break;
        default:
	    rt_kprintf("%s:%d, unsuported cases\n", __func__, __LINE__);
            break;
        }
...
const struct fwk_module module_pmic_voltage_domain = {
