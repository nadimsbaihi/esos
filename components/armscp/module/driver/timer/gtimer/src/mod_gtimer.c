...
{
    return hw_timer->TxCurrentValue;
#if 0
...
static int enable(fwk_id_t dev_id)
{
    ...
    ctx->hw_timer->TxControl |= DW_TIMER_TXCONTROL_ENABLE;
    /* enable interrupt. */
    ctx->hw_timer->TxControl &= ~DW_TIMER_TXCONTROL_INTMASK;
...
static int set_timer(fwk_id_t dev_id, uint64_t timestamp)
{
    ...
    ctx->hw_timer->TxLoadCount = (uint32_t)(timestamp & 0xFFFFFFFFUL);
#if 0
...
static int get_timer(fwk_id_t dev_id, uint64_t *timestamp)
{
    ...
    *timestamp = ctx->hw_timer->TxLoadCount;
#if 0
