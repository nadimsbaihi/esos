...
static struct mod_timer_config *gmod_timer;
#endif
...
static void timer_isr(int vector, void *ctx_ptr);
...
static void _configure_timer_with_next_alarm(struct timer_dev_ctx *ctx)
{
    ...
    if (alarm_head != NULL) {
        ...
        if (status != FWK_SUCCESS) {
            rt_kprintf("[Timer] %s @%d", __func__, __LINE__);
        }
...
static int alarm_stop(fwk_id_t alarm_id)
{
    ...
    if (!fwk_module_is_valid_sub_element_id(alarm_id)) {
    	rt_kprintf("%s:%d, err\n", __func__, __LINE__);
	while (1);
    }
    ...
    /* we should ensure that the timer is closed in the process context or timer interrupt
     * context.
     * */
                       ...
                       uintptr_t param)
{
    ...
    /* we should ensure that the timer is opened int process context */
    ctx = ctx_table + fwk_id_get_element_idx(alarm_id);
    alarm = &ctx->alarm_pool[fwk_id_get_sub_element_idx(alarm_id)];
...
#ifdef BUILD_USING_MTIMER
    gmod_timer = (struct mod_timer_config *)data;
#endif
...
static int timer_bind(fwk_id_t id, unsigned int round)
{
    ...
    if (round > 0) {
    	...
    	if (fwk_id_is_type(id, FWK_ID_TYPE_MODULE)) {
	    	status = fwk_module_bind(gmod_timer->alarm_id,
        		MOD_TIMER_API_ID_ALARM,
			&gmod_timer->alarm_api);
  	  	if (status != FWK_SUCCESS) {
    			rt_kprintf("%s:%d, bind mtimer failed\n", __func__, __LINE__);
			return FWK_E_ACCESS; 
   	 	}
    	}
#endif
...
#ifdef BUILD_USING_MTIMER
void mtimer_callback(uintptr_t param)
{
	rt_tick_increase();
}
#endif
...
static int timer_start(fwk_id_t id)
{
    ...
    rt_hw_interrupt_install(ctx->config->timer_irq,
		    timer_isr, (void *)ctx, RT_NULL);
    rt_hw_interrupt_umask(ctx->config->timer_irq);

#ifdef BUILD_USING_MTIMER
    /* enable the mtimer */
    gmod_timer->alarm_api->start(gmod_timer->alarm_id,
		    1000/*(ms)*/ / RT_TICK_PER_SECOND,
		    MOD_TIMER_ALARM_TYPE_PERIODIC,
		    mtimer_callback, (uintptr_t)NULL);
#endif
