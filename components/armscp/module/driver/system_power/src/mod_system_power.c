...
static void ext_ppus_set_state(enum mod_pd_state state)
{
    ...
    for (i = 0; i < system_power_ctx.config->ext_ppus_count; i++) {
        ...
        if (status != FWK_SUCCESS) {
            rt_kprintf("[SYS-POW] ext-ppu%i %s @%d", i, __func__, __LINE__);
        }
    }
}
...
static void soc_wakeup_handler(int vector, void *param)
{
    int status;
                                ...
                                const void *data)
{
    ...
    if (system_power_ctx.config->soc_wakeup_irq != FWK_INTERRUPT_NONE) {
        rt_hw_interrupt_install(system_power_ctx.config->soc_wakeup_irq,
                                     soc_wakeup_handler, RT_NULL, RT_NULL);
	rt_hw_interrupt_umask(system_power_ctx.config->soc_wakeup_irq);
    }
