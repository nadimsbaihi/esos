    ...
    size_t size)
{
    ...
    if (transport_type == MOD_TRANSPORT_CHANNEL_TRANSPORT_TYPE_NONE) {
    	rt_kprintf("%s:%d, hang\n", __func__, __LINE__);
	while (1);
    }
...
#else
        rt_kprintf("%s ERROR. IN-BAND MESSAGE NOT SUPPORTED!\n", MOD_NAME);
    ...
    bool request_ack_by_interrupt)
{
    ...
    if (transport_type == MOD_TRANSPORT_CHANNEL_TRANSPORT_TYPE_OUT_BAND) {
        ...
        buffer->reserved1 = 0;
#else
        rt_kprintf("%s ERROR. IN-BAND MESSAGES NOT SUPPORTED!\n", MOD_NAME);
        return FWK_E_SUPPORT;
#endif
    }
...
static int transport_signal_message(fwk_id_t channel_id)
{
    ...
    if (channel_ctx->config->transport_type ==
        ...
        if (!channel_ctx->out_band_mailbox_ready) {
            ...
            rt_kprintf("%s Out-band message not valid", MOD_NAME);
