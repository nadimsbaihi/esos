    ...
    unsigned int round)
{
    ...
    for (element_idx = 0; element_idx < fwk_mod_ctx->element_count;
        ...
        if (status != FWK_SUCCESS) {
            rt_kprintf(fwk_module_err_msg_func, status, __func__);
            return status;
        }
...
int fwk_module_start(void)
{
    ...
    if (fwk_module_ctx.initialized) {
        rt_kprintf(fwk_module_err_msg_func, FWK_E_STATE, __func__);
        return FWK_E_STATE;
    }
    ...
    rt_kprintf("[FWK] Module initialization complete!");
...
int fwk_module_bind(fwk_id_t target_id, fwk_id_t api_id, const void *api)
{
    ...
    if (status != FWK_SUCCESS) {
        rt_kprintf(fwk_module_err_msg_line, status, __func__, __LINE__);
        return status;
    }
