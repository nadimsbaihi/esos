...
static int radix_tree_extend(struct radix_tree_root *root, unsigned long index)
{
	...
	if (root->rnode) {
		do {
			if (!(node = rt_calloc(1, sizeof(struct radix_tree_node))))
				return -RT_ENOMEM;
...
{
	struct radix_tree_node *node = RT_NULL, *tmp, **slot;
	...
	while (height > 0) {
		if (*slot == RT_NULL) {
			...
			if (!(tmp = rt_calloc(1, sizeof(struct radix_tree_node))))
				return -RT_ENOMEM;
			*slot = tmp;
			if (node)
				node->count++;
...
void *radix_tree_delete(struct radix_tree_root *root, unsigned long index)
{
	...
	*pathp[0].slot = RT_NULL;
	while (pathp[0].node && --pathp[0].node->count == 0) {
		pathp--;
		*pathp[0].slot = RT_NULL;
		rt_free(pathp[1].node);
	}
