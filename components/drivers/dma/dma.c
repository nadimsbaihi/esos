...
#ifdef RT_USING_SMP
struct rt_spinlock dmac_nodes_lock;
#else
rt_base_t dmac_nodes_lock;
#endif
...
rt_err_t rt_dma_controller_register(struct rt_dma_controller *ctrl)
{
    ...
    rt_spin_lock_init(&dmac_nodes_lock);

    flags = rt_spin_lock_irqsave(&dmac_nodes_lock);
    rt_list_insert_before(&dmac_nodes, &ctrl->list);
    rt_spin_unlock_irqrestore(&dmac_nodes_lock, flags);
...
static struct rt_dma_controller *dtb_find_dma_controller(struct dtb_node *np,
        const char *name, struct fdt_phandle_args *args)
{
    ...
    if (name)
        index = dtb_node_property_match_string(np, "dma-names", name);
    ...
    dtb_node_parse_phandle_with_args(np, "dmas", "#dma-cells",
        index, args);

    flags = rt_spin_lock_irqsave(&dmac_nodes_lock);
    rt_list_for_each_entry(ctrl_tmp, &dmac_nodes, list)
    {
        if (ctrl_tmp->dev->node == args->np)
        {
            ctrl = ctrl_tmp;
            break;
        }
...
struct rt_dma_chan *rt_dma_chan_request(struct rt_device *dev, const char *name)
{
    ...
    struct fdt_phandle_args dma_args;
    struct rt_dma_controller *ctrl = RT_NULL;
    struct rt_dma_controller *ctrl_tmp;
    unsigned long flags;
    ...
    {
        fw_data = &dma_args;
        ctrl = dtb_find_dma_controller(dev->node, name, &dma_args);
    }
    else
    {
