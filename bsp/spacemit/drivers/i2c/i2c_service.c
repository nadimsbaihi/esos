...
#define RPMSG_SERV_NAME         "i2c-service"
...
static int i2c_irq_handler(int irq, void *arg)
{
	metal_irq_disable(SHUBI2C0_INT_REQ_IRQn);
...
static int rpmsg_endpoint_cb(struct rpmsg_endpoint *ept, void *data, size_t len, uint32_t src, void *priv)
{
	...
	if (strcmp(data, "startup") == 0) {
		/* reqeust i2c irq */
		ret = metal_irq_register(SHUBI2C0_INT_REQ_IRQn, i2c_irq_handler,
				NULL);
		if (ret) {
			rt_kprintf("Failed to register i2c irq handler\n");
			return -1;
		}

		metal_irq_enable(SHUBI2C0_INT_REQ_IRQn);
...
int rpmsg_i2c_service_init(void)
{
	...
	trigger_tid = rt_thread_create("i2c_trigger_irq_serivce",
			i2c_trigger_irq_thread_entry,
			NULL,
			2048,
			RT_THREAD_PRIORITY_MAX / 5,
			20);
	if (!trigger_tid) {
		rt_kprintf("Failed to create i2c service\n");
		return -1;
	}
...
INIT_APP_EXPORT(rpmsg_i2c_service_init);
