...
static int ccu_mix_enable(struct clk_hw *hw)
{
	...
	if (timeout_power > 1) {
		if (val == tmp)
			rt_kprintf("write clk_gate %s timeout occur, read pass after %d us delay\n",
			clk_hw_get_name(&common->hw), timeout_power);
		else
			rt_kprintf("write clk_gate  %s timeout after %d us!\n", clk_hw_get_name(&common->hw), timeout_power);
	}
...
static unsigned char ccu_mix_get_parent(struct clk_hw *hw)
{
	struct ccu_mix *mix = hw_to_ccu_mix(hw);
	struct ccu_common * common = &mix->common;
...
static int ccu_mix_set_parent(struct clk_hw *hw, unsigned char index)
{
	struct ccu_mix *mix = hw_to_ccu_mix(hw);
	struct ccu_common * common = &mix->common;
	struct ccu_mux_config *mux = mix->mux;
